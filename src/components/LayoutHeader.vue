    ...
    <div class="container">
      ...
      <div class="flex items-center justify-between -mx-2 sm:-mx-4">
        <div class="flex flex-col items-center px-2 mr-auto sm:px-4 sm:flex-row">
          ...
          <div v-if="settings.nav.links.length > 0" class="hidden ml-2 mr-5 sm:flex items-center sm:ml-2">
            <g-link
              v-for="link in settings.nav.links"
              :key="link.path"
              :to="link.path"
              class="block p-1 font-medium nav-link text-ui-typo hover:text-ui-primary sm:ml-4 md:whitespace-no-wrap"
            >
              {{ link.title }}
            </g-link>
          </div>
